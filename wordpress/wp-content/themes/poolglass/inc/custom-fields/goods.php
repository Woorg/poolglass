...
add_action('init', 'register_post_goods');

function register_post_goods() {
...
function crb_goods() {
	Container ::make('post_meta', __('Вид остекления'))
	          -> where('post_type', '=', 'goods')
	          -> add_tab(__('Отображение на главной странице'), [
		          Field ::make('image', 'goods_img', __("Картинка товара"))
		                -> set_value_type('url'),
		          Field ::make('rich_text', 'goods_desc', __("Описание товара")),
		          Field ::make('text', 'goods_meta_title', __("SEO TITLE")),
		          Field ::make('text', 'goods_back_link', __('Техническое поле для ссылок (Не трогать!)'))
		                -> set_default_value('tovari'),
	          ])
	          -> add_tab(__('Настройка отображения записи'), [
		          Field ::make('text', 'post_main_title', __('Заголовок статьи')),
		          Field ::make('complex', 'goods_post_item', __('Данные статьи'))
		                -> setup_labels([
		                ...
		                -> add_fields(array(
			                ...
			                Field ::make('separator', 'goods_separator', __('Данные по тексту')),
